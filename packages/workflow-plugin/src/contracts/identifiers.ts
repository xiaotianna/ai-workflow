...
export const PLUGIN_NODE_TYPE_PATTERN = /^plugin:[a-z][a-z0-9-]*\/[a-z][a-z0-9-]*\/[a-z][a-z0-9-]*$/
...
export const pluginIdSchema = z
  ...
  .min(1)
  .max(64)
  .regex(PLUGIN_ID_PATTERN, '插件 ID 只能使用小写字母、数字和连字符')
...
export function createPluginNodeType(publisher: string, pluginId: string, nodeKey: string) {
  return `plugin:${publisher}/${pluginId}/${nodeKey}` as const
}
