...
export async function devPlugin(options: DevPluginOptions = {}): Promise<void> {
  ...
  const rootDirectory = initialBuild.package.rootDir
  const outDir = resolvePackageOutputDirectory(rootDirectory, options.outDir)
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      const requestPath =
        url.pathname === '/' ? 'plugin.manifest.json' : decodeURIComponent(url.pathname.slice(1))
      const filePath = resolve(outDir, requestPath)
