...
- LLM 等按消息组织内容的场景使用 UI 包 `TiptapEditor` 作为编辑核心，业务层负责角色、变量
  选择、删除和新增消息；编辑器外壳不得复制 Tiptap 初始化、token 插入或字符串序列化逻辑。
- 消息卡片沿用节点代码字段的轻量外壳，默认使用 `bg-input`、1px 描边占位和 `rounded-lg`；
  工具栏固定为 36px 高并使用低对比分隔线，角色选择与图标按钮作为内嵌控件，不显示独立输入框
  轮廓。角色 Trigger 使用 24px 高度和弱化文字色，角色菜单使用紧凑宽度、28px 选项与浅色
  Hover / 选中背景，继续保留 Select 原有的主色勾选标记。Hover 时使用
  `border-input-focus` 语义色；任一内部控件获得焦点时切换为
  `bg-background`，外描边使用 `--primary` 到 `--info` 的低范围流动渐变表达当前编辑目标。
  描边只占用既有 1px 空间，不得改变尺寸或使用 `ring-*`、外扩阴影。
- 流动描边必须遵循系统 reduced motion；减少动态效果时保留静态渐变边框。错误态优先使用
  `border-destructive`，不与流动描边叠加。校验文案出现或消失时，消息项只做位置布局动画，
  不对编辑卡片做尺寸缩放，避免输入内容与边框抖动。禁用态阻止角色、变量、删除与内容编辑。
- 消息内工具区只展示当前任务真实需要的操作。LLM 上下文当前只保留变量与删除，不显示计数、
  AI、模板开关、复制或放大入口；所有图标按钮提供准确的无障碍名称。
- 上下文字段标题的新增按钮与输入、输出变量字段统一使用 Ghost `icon-xs` 按钮、弱化前景色和
  16px Plus 图标，不在同级动态字段之间使用不同的图标尺寸或默认颜色。
