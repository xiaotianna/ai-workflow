...
export function useWorkflowTestRun(appId: string) {
  const inFlightRef = useRef(false),
    abortControllerRef = useRef<AbortController | undefined>(undefined),
    activeRunIdRef = useRef<string | undefined>(undefined),
    pauseInFlightRef = useRef(false),
    pauseRequestedRef = useRef(false),
    pausedResultRef = useRef<StudioWorkflowTestRunDto | undefined>(undefined),
    lifecyclePauseRequestedRef = useRef(false),
    [activeRunId, setActiveRunId] = useState<string>(),
    [pending, setPending] = useState(false),
    [pausing, setPausing] = useState(false),
    [runResult, setRunResult] = useState<StudioWorkflowTestRunDto>(),
    [nodeExecutionStatuses, setNodeExecutionStatuses] = useState<WorkflowNodeExecutionStatuses>({}),
    pauseActiveRun = useCallback(
      ...
      async (request: WorkflowTestRunRequest): Promise<StudioWorkflowTestRunDto> => {
        ...
        try {
          const runState: {
            ...
            handleEvent = (event: StudioWorkflowTestRunSseEvent) => {
    ...
    pause = useCallback(() => pauseActiveRun(false), [pauseActiveRun])
