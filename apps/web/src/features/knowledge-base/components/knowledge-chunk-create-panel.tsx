...
interface KnowledgeChunkCreatePanelProps {
  saving: boolean
  sequence: number
  onClose: () => void
...
export function KnowledgeChunkCreatePanel({
  saving,
  sequence,
  onClose,
  onSave,
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <header className="flex shrink-0 items-start gap-3 px-4 pt-4">
        <div className="min-w-0">
          <h2 className="text-sm leading-5 font-semibold">添加分段</h2>
          <p className="text-muted-foreground mt-0.5 flex items-center gap-1.5 text-[11px] leading-4 font-medium">
            <span className="flex items-center gap-1">
              ...
              分段-{String(sequence).padStart(2, '0')}
            </span>
