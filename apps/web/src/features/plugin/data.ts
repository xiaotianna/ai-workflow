...
const seedPlugins: PluginListItem[] = [
  {
    id: 'github',
...
const generatedPluginDescriptions = [
  '连接外部服务并在工作流中同步数据',
  '自动化重复任务并简化日常操作',
  '提供可组合的 AI 能力与工作流集成',
  '统一处理信息检索、转换与输出',
...
function createGeneratedPlugin(index: number): PluginListItem {
  ...
  return {
    ...
    description: `${title} - ${generatedPluginDescriptions[index % generatedPluginDescriptions.length]}`,
    tags: [...new Set(tags)],
    icon: extraIcons[index % extraIcons.length] ?? Wrench,
  }
}
...
const usedPluginIds = new Set(['github', 'google-search', 'openai'])
const ownedPluginIds = new Set(['web-scraper', 'workflow-bundle'])
...
function matchesMockPluginFilter(plugin: PluginListItem, filterId: PluginFilterId) {
  switch (filterId) {
    case 'installed': {
      return installedPluginIds.has(plugin.id)
    }
    case 'used': {
      return usedPluginIds.has(plugin.id)
    }
    case 'mine': {
      return ownedPluginIds.has(plugin.id)
    }
    default: {
      return true
    }
...
export function filterMockPlugins(search: string, filterId: PluginFilterId) {
  const normalizedSearch = search.trim().toLowerCase()

  return mockPlugins.filter((plugin) => {
    const matchesFilter = matchesMockPluginFilter(plugin, filterId)
    const matchesSearch =
      !normalizedSearch ||
