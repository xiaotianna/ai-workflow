...
- 知识库已落地管理、设置、Source/Version/Head/Attempt/Projection 事实模型、RabbitMQ + Outbox
  ...
  删除和供应商类型修改会被阻止；模型 ID 只在关联索引构建或文档 Embedding 期间禁止修改，处理完成后允许修改。分段配置变更只标记旧文档待更新，不自动覆盖 Chunk。
  失败索引可通过显式重建接口创建新代际并重新投递 Outbox；重建会重新处理启用的失败文档，成功
  Head 随新代际激活后恢复文档为 `READY`，禁止原地复活失败代际或并发重复构建。
