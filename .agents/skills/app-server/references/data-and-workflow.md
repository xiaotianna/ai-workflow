...
- 统一应用镜像的 Server 构建阶段只为 `prisma:generate` 提供指向 localhost 的无效占位 `DATABASE_URL`，
  该命令不连接数据库；最终容器仍由 `docker-entrypoint.sh` 从 secrets 组装真实连接串后执行 migration
  和启动服务，不能把生产连接串作为 Docker build arg 或写入镜像层。
