...
- 文档详情的“添加分段”复用分段编辑面板的浮层外壳、正文表单密度和校验规则；提交后通过真实
  分段接口进入当前文档的版本与投影链路，不在浏览器生成永久分段。提交成功后必须使用接口返回的真实分段 ID
  立即在当前列表乐观展示，投影完成前标记为“更新中”并禁用选择、编辑、删除和启停；轮询确认 Head 切换后用真实列表结果解除禁用，
  失败时回滚该乐观项。文档正在处理或请求提交中时禁止重复添加。
- 文档元数据标注在右侧信息区原位切换，不打开全屏 Dialog。文档尚无元数据时才展示说明空状态和
  “开始标注”；已有持久化元数据时直接展示紧凑的只读键值列表，列宽、行高和间距与下方文档信息保持一致，
  并在头部提供“编辑”入口。编辑态头部保留取消、
  保存和全宽“添加元数据”入口；字段值随文档持久化，String、Number、Time 使用匹配类型的输入控件。
  元数据字段属于知识库级目录，选择浮层支持搜索与新建；字段行只在 Hover 或内部聚焦时显示编辑、
  删除按钮，底部只保留“新建元数据”，不提供独立“管理”入口。字段删除必须二次确认，并同时清理
  所有文档的对应值；文档编辑区的删除按钮只移除当前文档对该字段的标注。
- 添加文件固定使用“选择数据源 → 文本分段与清洗 → 处理并完成”三步。步骤 2 左侧选择“通用 / Q&A /
  ...
  当前知识库配置的只读摘要和设置入口。未配置可用 Embedding 模型时仍允许生成临时分段预览，
  但必须禁用“保存并处理”，展示前往知识库设置的入口；后端正式上传接口继续做同一强制校验。
  步骤 3 的标题和每个文件状态必须消费服务端返回的
  `PROCESSING / READY / FAILED`，分别展示处理中、完成和失败语义；不得在上传接口返回后固定显示
  “嵌入已完成”，只有服务端返回 `READY` 才能显示“索引已就绪”。
...
- 文档表格的头部复选框只全选当前页，筛选、分页或每页数量变化后清空选择。当前页存在已选文档时，
  在分页器上方展示与分段列表共用的 `KnowledgeSelectionActions`，提供批量启用、禁用、重新索引、删除和取消；
  批量启停、重新索引与删除必须持久化，重新索引沿用单文档的处理中状态和短轮询，删除前必须二次确认；
  请求期间禁止重复提交、修改选择或触发行内操作。
...
- `DetailLayout.sidebarFooter` 用于详情侧栏底部、账户菜单上方的业务信息区；知识库在这里展示真实
  文档数、去重后的关联应用数和 API 访问入口，不把该区域复制进通用账户菜单。API 下拉在触发器
  上方且与触发器等宽，展示真实启用状态，支持启停、管理 `kb-live-` 密钥和打开 RAG API 文档；
  状态点使用语义色 40% 背景与边框、8px 方圆形和轻阴影。
