...
- 插件详情的安装主操作使用固定 `h-9` 的 `overflow-hidden rounded-lg` 分体结构，两侧子控件均使用 `h-full`
  消除 Button 与 Select 默认盒模型差异。左侧保持 `w-[135px]`，未安装或有更新时
  使用主按钮，已是当前版本时使用非禁用态的主色状态区显示“已安装”或“已禁用”，不得借用主按钮 Disabled 的浅色视觉。
  已安装时，右侧使用 `size-9` 的 Select Trigger，通过 `border-primary-foreground/15 border-l` 分隔，选项提供启用或禁用以及卸载；
  卸载必须二次确认。详情摘要不提供独立下载按钮。
