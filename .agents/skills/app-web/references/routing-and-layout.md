...
- `/app/:id` 是与首页布局并列的根级应用布局，不渲染首页侧栏内容；应用布局同样在页面中直接组合 `LayoutSidebar`，注入自己的 header 和导航项，由内容区承载子路由。
...
- 首页与应用详情布局统一使用 `h-svh overflow-hidden p-1` 的外层容器，主内容区使用 `min-h-0 overflow-auto`，确保共享侧栏及底部账户菜单在路由切换时保持相同位置。
