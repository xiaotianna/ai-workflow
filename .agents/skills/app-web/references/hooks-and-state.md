...
- 插件发布成功后刷新当前查询，使新版本及其可见范围立即反映在列表中；列表不得回退到 Mock 数据
  或客户端集合筛选。
- 安装或升级成功后由 `usePlugins.applyInstallation` 按平台插件 UUID 更新当前列表项的安装版本、
  权限快照和更新状态；只有首次安装增加当前卡片安装数，升级不得重复计数。
...
- 当前节点的变量候选由 `features/workflow/utils/get-available-variables.ts` 统一合并 Core 系统
  ...
  相同裸 Key 而不冲突。普通节点的输入绑定、输出值映射与配置字段共用该候选集合，输出映射
  不额外加入当前节点或下游变量。Web 不维护另一份系统变量清单或环境变量命名空间。
